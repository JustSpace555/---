...
data class Matrix (val array: MutableList<MutableList<Double>>) {
	...
	val norm: Double by lazy { sqrt(array.flatten().sumOf { it * it }) }
	...
	fun replaceRow(i: Int, otherRow: List<Double>) {
		if (otherRow.size != columns)
			throw IllegalArgumentException("New row must have equal size as other rows in matrix")
		array[i] = otherRow.toMutableList()
	}

	fun replaceColumn(j: Int, otherColumn: List<Double>) {
		if (otherColumn.size != rows)
			throw IllegalArgumentException("New column must have equal size as other columns in matrix")

		for (i in array.indices) array[i][j] = otherColumn[i]
	}
	...
	fun applyOnEach(operation: (Double) -> Double) = of(array.flatten().map { operation(it) }.chunked(columns))

	fun toPrint(): String = array.joinToString("\n") { row -> row.joinToString("\t") }
}