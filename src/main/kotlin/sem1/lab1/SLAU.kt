...
class SLAU(private val matrix: Matrix) {
	...
	fun luDecomposition(): Pair<Matrix, Matrix> {
		if (!matrix.isSquare) throw IllegalStateException("sem1.lab1.Matrix is not square")
	...
	private inner class Exercise3(private val inputEps: Double) {
		...
		init {
			...
			if (matrix.rows != matrix.columns - 1)
				throw IllegalStateException("sem1.lab1.Matrix should be square for this method")
		...
		fun zeidelMethod(): Triple<Matrix, Int, Double> {
			...
			do {
				...
				epsI = if (alpha.norm < 1)
					C.norm / (1 - alpha.norm) * (x - prevX).applyOnEach(::abs).norm
				else (x - prevX).applyOnEach(::abs).norm
				i++
			} while (epsI > inputEps)
